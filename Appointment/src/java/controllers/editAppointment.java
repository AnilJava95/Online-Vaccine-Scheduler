...
public class editAppointment extends HttpServlet
{
		...
		throws ServletException, IOException
	{
		...
		if (status > 0) {
			switch (timeSlot) {
				case "1": date.setTimeSlot1("true"); break;
				case "2": date.setTimeSlot2("true"); break;
				case "3": date.setTimeSlot3("true"); break;
				case "4": date.setTimeSlot4("true"); break;
				case "5": date.setTimeSlot5("true"); break;
			}
			AppointmentDao.editDate(date);
			
			request.getRequestDispatcher("checkSymptom.jsp").include(request, response);
			out.print("<br> ----------------------------- Edit appointment.");
		}
		else {
			request.getRequestDispatcher("findAppointmentForm.jsp").include(request, response);
			out.print("----------------------------- Old appointment was unable to be deleted.");
		}
